...
final class Controller {
    ...
    private Command[] _commands = {
        ...
        new Command("manual\\s+(?i)(white|black)$", this::setManual),
        new Command("auto\\s+(?i)(black|white)$", this::setAuto),
        new Command(String.format("%s-%s\\(%s\\)|%s\\s+%s\\s+%s",
                SQ, SQ, SQ, SQ, SQ, SQ), this::move)
    };
    ...
    /** Set the auto player. */
    /** @param side side is nothing. */
    private void setAuto(Matcher side) {
        if (side.group(1).toLowerCase().equals("white")) {
            _white = _autoPlayerTemplate.create(WHITE, this);
        } else if (side.group(1).toLowerCase().equals("black")) {
            _black = _autoPlayerTemplate.create(BLACK, this);
        }
    }
    /** Set the manual player. */
    /** @param side is something. */
    private void setManual(Matcher side) {
        if (side.group(1).toLowerCase().equals("white")) {
            _white = _manualPlayerTemplate.create(WHITE, this);
        } else if (side.group(1).toLowerCase().equals("black")) {
    ...
    /** Set the move method. */
    /** @param moves is move. */
    private void move(Matcher moves) {
        String a = moves.group();
        Move path = Move.mv(a);
        _board.makeMove(path);
        Piece turn = _board.turn();
        Iterator<Move> remaingmoves = _board.legalMoves(turn);
        if (!remaingmoves.hasNext()) {
