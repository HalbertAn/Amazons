...
class BoardWidget extends Pad {
    ...
    static final Color
        ...
        CLICK_SQUARE_COLOR = new Color(103, 61, 225);
    ...
    private void drawGrid(Graphics2D g) {
        ...
        for (int i = 1; i < 100; i = i + 2) {
            g.fillRect(cx(sq(i)), cy(sq(i)), SQUARE_SIDE, SQUARE_SIDE);
            int remain = i % 10;
            if (remain == 9) {
                i--;
            }
    ...
    public synchronized void paintComponent(Graphics2D g) {
        ...
        for (; sq.hasNext();) {
            Square cur = sq.next();
            Piece ps = _board.get(cur);
            if (clickedsquare.contains(cur)) {
                g.setColor(CLICK_SQUARE_COLOR);
            }
            if (ps == BLACK) {
            ...
            if (ps == SPEAR) {
                ...
                g.fillRect(cx(cur), cy(cur), SQUARE_SIDE, SQUARE_SIDE);
            }
    ...
    private void click(Square s) {
        if (clickedsquare.size() == 1) {
            if (_board.isLegal(clickedsquare.get(0), s)) {
                clickedsquare.add(s);
                move += String.format("%s ", s);

            }
        } else if (clickedsquare.size() == 2) {
            String cur = move;
            cur += String.format("%s ", s);
            if (_board.isLegal(Move.mv(cur.trim()))) {
                clickedsquare = new ArrayList<>();
                _commands.offer(cur);
                move = "";
            }
        } else {
    ...
    /** collect clicked square. */
    private ArrayList<Square> clickedsquare = new ArrayList<>();
    /** moving path. */
    private String move = "";
